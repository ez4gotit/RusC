...
Автор и основатель проекта — **[ez4gotit](https://github.com/ez4gotit)**.
...
- команды `собрать`, `запустить`, `раскрыть`, `осмотреть`, `проверить`,
  `здравие`, `версия` и `право`;
- кириллица в исходных файлах, именах и выводе;
- объектные `роды`, наследование, сокрытие, зачины, умения и порождение объектов;
...
Объектная модель описана в [справочнике ООП](docs/oop.md). Рабочий пример:
[`examples/oop.rus`](examples/oop.rus).

## Visual Studio Code

Официальное расширение находится в [`editors/vscode`](editors/vscode). Оно
добавляет:

- подсветку современного, древнерусского и объектного синтаксиса;
- автоотступы, сворачивание блоков и комментарии через `#`;
- сниппеты для программ, циклов, родов, зачинов и умений;
- дерево родов, черт и умений в Outline;
- диагностику `rusc` в панели Problems;
- команды сборки, запуска, раскрытия C# и проверки EXE.

Установка готового VSIX:

```powershell
code --install-extension .\editors\vscode\ruslang-0.1.0.vsix
```

Расширение автоматически ищет компилятор в
`artifacts/rusc/win-x64/rusc.exe`. Другой путь задаётся настройкой
`ruslang.compilerPath`.

Для пересборки пакета расширения:
...
Показывает автора, лицензию компилятора и свободы программ пользователя:
...
- `docs/oop.md` — роды, наследование, сокрытие и полиморфизм;
- `docs/releasing.md` — выпуск автономного компилятора;
- `docs/spikes/appended-payload.md` — результаты Windows spike.

## Лицензия

RusLang распространяется по модели
**AGPL-3.0-or-later с RusLang Output Exception 1.0**:

- форки и изменения разрешены при сохранении авторства `ez4gotit`;
- изменённый форк обязан оставаться открытым и не выдавать себя за официальный;
- закрытый сетевой сервис на изменённом компиляторе также должен предоставить
  соответствующий исходный код;
- программы, написанные на RusLang, и созданные `.exe` принадлежат их авторам и
  могут быть закрытыми, коммерческими и распространяться под любой лицензией.

Полные условия находятся в [`LICENSE`](LICENSE), сведения об авторстве — в
[`NOTICE`](NOTICE).
